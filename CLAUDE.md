...
4. For each returned person URL:
   ...
   - Accumulate a stub record — `status: "new"`, `first_seen_run: <today>`, plus any fields returned by the search API (at minimum `name`, `headline`, `location` — these are critical when enrichment is disabled) — into an in-memory batch delta keyed by URL. **Do not read or write `leads.json` inside this per-URL loop.**
5. Once every query in the current batch (`search.max_search_queries_per_run` queries) has been processed, write the accumulated delta to a small temp JSON file and run `node standalone/bin/merge-leads.mjs <tempfile>` once via Bash — this merges the whole batch's stubs into `leads.json` with a single atomic read-modify-write, inserting new URLs and leaving existing entries alone. `leads.json` is a multi-megabyte, ever-growing ledger (thousands of leads); reading and rewriting it once per lead instead of once per batch is the single biggest cause of pipeline slowness and disk churn — never loop `leads.json` writes per-URL. (`seen.json` stays write-through per-URL as above — it's small, and per-URL durability there is what keeps the exclusion set correct if a batch is interrupted.)
6. On `rate_limited`: wait `rate_limit.retry_delay_seconds`, retry up to `rate_limit.max_retries`. If still failing, log, skip that query, continue to next.
...
5. Build one delta keyed by URL — `score`, `classification`, `score_rationale`, `status: "classified"`, `last_updated` — for every result in the returned array, then write it to a small temp JSON file and run `node standalone/bin/merge-leads.mjs <tempfile>` once via Bash to merge the whole batch into `leads.json` in a single atomic read-modify-write. Same rule as Step 2: never loop per-result Reads/Edits against `leads.json`.
...
- Step 5: writes `state/pending_approvals/<date>-inmail-ready.json` (entries never removed, only decision fields filled in — same invariant as `notes-ready.json`). No email notification for this phase — see [[feedback_invite_recovery_no_email]] — the approval file is the review surface.
- Step 6: moves fully-resolved CSVs to `state/imports/processed/` (move, never delete); finalizes the run.

**`/send-recovery-inmail`** — Step 7 in this phase's numbering: re-checks `get_inmail_credits`, scans `*-inmail-ready.json` for `decision: "approved"` entries whose lead is still `status: "inmail_queued"` with `inmail_sent_at` null, sends via `send_inmail` (connectsafely: no subject support — see the operations table note above; linkedapi: subject supported), sets `status → "inmail_sent"`. No email notification — check `run_log.json` for the delivery summary counts.
...
- **`pending_approvals/`**: one timestamped file is created per run — never deleted, and the file itself is never merged into another file. `/pipeline-maintenance` may *move* fully-decided files to `pending_approvals/archive/`; nothing else relocates them. In entry-style files (`-hot/-warm/-cold/-connection/-notes-ready/-followup/-warmup-comments/-inmail-ready`) entries are never removed — only decision fields are filled in. Entries older than `approval_timeout_days` with null decisions are expired by Step 6a — it stamps `decision: "rejected"` on them (they remain in the file), so fully-decided files can eventually be archived by `/pipeline-maintenance`.
  ...
  In `*-links.json` files the invariant is **narrower than it used to be**: the file itself is still never merged, and hot/warm/expired bookkeeping still never leaves the file (tier → `expired` by Step 6a remains a pure in-file move). But **`approved`, `cold`, and deletion-when-empty are three deliberate exceptions**: a URL cut into `approved` moves out of the links file entirely into the single shared `approved-queue.json` (a flat array of full URLs); a URL still sitting in `cold` past the current run moves out into the single shared `cold-registry.json` (a flat array of bare slugs) via the Cold Sweep at the start of the next abbreviated run. Links files are therefore written with only `hot`, `warm`, and `cold` keys — no `approved` key — and `cold` is expected to end each run cycle empty. Once `hot`, `warm`, and `cold` are **all** empty and the file is older than `/pipeline-maintenance`'s `days` threshold, sweep (a) **deletes the file outright instead of archiving it** — an all-empty links file carries no information that isn't already durable in `leads.json` (every URL it ever listed has its outcome recorded via `approval_surfaced_at`/`approval_decision`/`status`), so moving the husk to `archive/` would be pure file-count noise, not data preservation. This is the one file type `/pipeline-maintenance` deletes rather than moves; every other file type it touches is still archived, never deleted.
...
4. Steps that partially completed (e.g., Step 3 enriched 10 of 20 leads before crashing) are safe to re-run because: enriched leads have non-null `name`; `seen.json` already has all discovered URLs; and `leads.json` writes (via `standalone/bin/merge-leads.mjs`, see Steps 2 and 4) are batched per search/classification batch rather than fully in bulk for the whole step — so at most one in-flight batch's worth of `leads.json` work is ever redone, and redoing it is harmless: `merge-leads.mjs`'s upsert is idempotent (re-merging a URL that's already present just overwrites it with the same values), and `seen.json`'s per-URL write-through means no URL is ever silently skipped or double-processed across the crash boundary.
