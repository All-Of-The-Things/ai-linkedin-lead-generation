...
2. Executes pipeline **Step 6a** — Compose Connection Note Drafts:
   - Scans all `state/pending_approvals/*-connection.json` files for entries where `decision: "approved"` and `note_draft` is null: invokes `agents/message-composer.md` to draft a personalized connection note.
   - Writes `note_draft` and `note_decision: null` back to the entry in the same file it was read from.
...
- At least one entry across all `state/pending_approvals/*-connection.json` files must have `decision: "approved"` and `note_draft: null`. If all approved entries already have drafts, nothing will be composed and you'll get a warning.
- `RESEND_API_KEY` must be set in the environment for the email notification to send.

## After running

Check your email — it shows each draft note alongside the lead. Open the relevant `state/pending_approvals/*-connection.json` file(s) and for each entry:
