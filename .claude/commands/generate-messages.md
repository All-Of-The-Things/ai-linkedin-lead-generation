...
**Phase 2 of the LinkedIn pipeline.** Run this after you have reviewed the approval files in `state/pending_approvals/` — either by setting `decision: "approved"` in the classification files, or by moving URLs into `state/pending_approvals/approved-queue.json` (abbreviated runs).
...
2. Executes pipeline **Step 6a** — Compose Connection Note Drafts:
   - Scans all approval files (`*-hot.json`, `*-warm.json`, `*-cold.json`, `*-connection.json`, `*-links.json`) plus the shared `approved-queue.json` and `cold-registry.json` for approved leads without drafts. URLs in `approved-queue.json` count exactly as `decision: "approved"`.
   - Expires links-file hot/warm URLs left undecided past `approval.approval_timeout_days` (moved to that file's `expired` array, lead marked rejected), and separately expires `cold-registry.json` slugs the same way using each lead's own `approval_surfaced_at` (removed from the registry once expired). Move a URL into `approved-queue.json` to rescue it later. Stale undecided classification/legacy entries expire the same way: `decision` is stamped `"rejected"` and the lead marked rejected (`approval_decision: "expired"`), so decided files can later be archived by `/pipeline-maintenance`.
   ...
   - Writes `note_draft` back to the source classification file (approved-queue leads have no per-entry fields — their drafts live only in the notes-ready file, and they're removed from `approved-queue.json` once drafted).
